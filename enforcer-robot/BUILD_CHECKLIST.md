...
- [ ] ⚠️ **Aim the camera at the hands, not the head.** Measured on the first
      desk recording: at 53.5° a phone *held in the hand* falls outside the
      frame while a phone *on the desk* stays in it — the exact inverse of
      what the robot needs, and why phone precision fell 0.90 → 0.23 under the
      crop. Frame the hands-and-desk zone, then re-record and re-run
      `evaluate.py` before committing the mount to the shell
